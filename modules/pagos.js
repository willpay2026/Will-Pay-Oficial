// ==========================================
// WILL-PAY VENEZUELA - SISTEMA DE PAGOS ELITE
// LEGADO: Wilfredo Donquiz para Wilyanny
// ==========================================

let miSaldo = 7560.00; 
let nombreUsuario = "YESICA"; 
let scannerQR; 
let modoAbierto = null;

// 🔊 FUNCIÓN DEL BEEP PROFESIONAL (Sonido de éxito)
const sonarBeep = () => {
    try {
        ...
        <div style="text-align:center; animation: fadeIn 0.5s;">
            ...
            <div style="background: #000; border: 2px solid #ffcf40; border-radius: 25px; padding: 25px; margin-bottom: 15px; box-shadow: 0 10px 20px rgba(0,0,0,0.4);">
                ...
                <div style="display:flex; gap:10px; justify-content:center;">
                    <button onclick="window.toggleModulo('RECARGA')" style="flex:1; background: #ffcf40; border:none; padding:12px; border-radius:12px; font-weight:bold; font-size:0.7rem; cursor:pointer;">RECARGAR</button>
                    <button onclick="window.toggleModulo('RETIRO')" style="flex:1; background: #1a1a1a; color:#fff; border:1px solid #333; padding:12px; border-radius:12px; font-weight:bold; font-size:0.7rem; cursor:pointer;">RETIRAR</button>
                </div>
            </div>
            ...
            <div id="area-operacion" style="background: #0a0a0a; border: 1px solid #222; border-radius: 20px; padding: 20px; min-height: 250px; display: flex; flex-direction: column; align-items: center; justify-content: center; position:relative; overflow:hidden;">
...
// 🔄 LÓGICA DE ABRIR/CERRAR (TOGGLE)
window.toggleModulo = (tipo) => {
    const area = document.getElementById('area-operacion');
    if (modoAbierto === tipo) {
        area.innerHTML = `<p style="color: #333; font-size: 0.7rem;">ESPERANDO OPERACIÓN</p>`;
        modoAbierto = null;
    } else {
        modoAbierto = tipo;
        area.innerHTML = `
            ...
            <div style="background:#111; padding:10px; border-radius:10px; margin-bottom:10px; border:1px solid #222; text-align:left; width:100%;">
                 <p style="color:#888; font-size:0.6rem; margin:0;">${tipo === 'RECARGA' ? 'PAGO MÓVIL DESTINO:' : 'MÉTODO DE SALIDA:'}</p>
                 <p style="color:#fff; font-size:0.7rem; margin:0;"><b>04126602555 | Banesco</b></p>
            </div>

            <input type="number" id="monto-input" placeholder="Monto Bs." style="width:100%; padding:12px; background:#000; border:1px solid #333; color:#fff; border-radius:10px; margin-bottom:10px;">
            <input type="number" id="ref-input" placeholder="${tipo === 'RECARGA' ? 'Nro. Referencia' : 'Cédula Destino'}" style="width:100%; padding:12px; background:#000; border:1px solid #333; color:#fff; border-radius:10px; margin-bottom:15px;">
            
            <button onclick="window.confirmarOperacion('${tipo}')" style="width:100%; background:#ffcf40; color:#000; border:none; padding:12px; border-radius:10px; font-weight:bold; cursor:pointer;">${tipo === 'RECARGA' ? 'NOTIFICAR A WILFREDO' : 'SOLICITAR RETIRO'}</button>
        `;
    }
};

// ✅ CONFIRMACIÓN DE RECARGA/RETIRO
window.confirmarOperacion = (tipo) => {
    const m = document.getElementById('monto-input').value;
    const r = document.getElementById('ref-input').value;
    if(!m || !r) return alert("Por favor complete los datos.");
    
    alert(`✅ ${tipo} REPORTADA\n\nWilfredo Donquiz verificará la información para procesar su saldo.`);
    window.mostrarDashboardUsuario();
};

// 📸 LÓGICA DE CÁMARA Y PAGOS
...
// 📑 GENERACIÓN DE TICKET AUDITABLE
window.generarTicket = (datos) => {
    ...
    area.innerHTML = `
        <div style="background: #fff; color: #000; padding: 20px; border-radius: 10px; width: 100%; font-family: 'Courier New', Courier, monospace; text-align: left; box-shadow: 0 10px 30px rgba(0,0,0,0.5); animation: slideUp 0.4s;">
            ...
            <p style="font-size: 0.7rem; margin: 5px 0;"><b>FECHA/HORA:</b> ${fecha}</p>
            <p style="font-size: 0.7rem; margin: 5px 0;"><b>CLIENTE:</b> ${nombreUsuario}</p>
            <p style="font-size: 0.7rem; margin: 5px 0;"><b>ESTATUS:</b> <span style="color: green; font-weight:bold;">CERTIFICADO ✅</span></p>
            <hr style="border: 0.5px dashed #000; margin: 10px 0;">
            <center>
                <p style="font-size: 0.8rem; margin: 5px 0;">TOTAL PAGADO</p>
                <h2 style="margin: 0; font-size: 2rem;">Bs. ${parseFloat(monto).toFixed(2)}</h2>
            </center>
            <hr style="border: 0.5px dashed #000; margin: 10px 0;">
